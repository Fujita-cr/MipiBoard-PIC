...
//20240613 V17・・・画面非表示とサウンド問題の対策として下記の対応
...
bool TC358870_Bootflg = false;       //Windows起動完了フラグ

#define TC358870_BOOT_WAIT 150    //PCが起動するまでの待ち時間 150に設定したときは10msタイマーx20回(TC358870_CheckAll)x150回=30000ms(30秒)
                                  //NUC11での液晶非表示、サウンド不具合対策
                                  //TC358870_BOOT_WAIT=0のときは、最初の1回(200ms)だけ待ってHDMIの初期化が始まる
//#define TC358870_BOOT_WAIT 0 

///////////////////////////////////////////////////////////////////////////////
//           プログラムのバージョン情報                                        //
//           プログラムを更新する場合に必ず書き換えること！                     //
///////////////////////////////////////////////////////////////////////////////
const uint16_t   m_version = 1701;  //プログラムのバージョン情報 テスト版のときは100XXとかにする
///////////////////////////////////////////////////////////////////////////////
...
int main(void)
{
    ...
    //Camera_Reset_Release(); //Cypressカメラリセット解除
...
//        __delay_ms(100);
        ...
        if(TC358870_Timer >= 1){    //10ms毎にチェックする。
            TC358870_CheckAll++;
            if (TC358870_CheckAll > 20){    //200ms毎に処理
                if (TC358870_Bootflg || TC358870_BootCount > TC358870_BOOT_WAIT){
                    tc358870(true); //バックライト制御などは200msに1回
                }else if (!TC358870_Bootflg && TC358870_BootCount <= TC358870_BOOT_WAIT){
